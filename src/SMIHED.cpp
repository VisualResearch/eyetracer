...
SMIHED::SMIHED (LogFileWriter *log) 
...
bool SMIHED::parseDataFile (string file_path, list<Data*> &list_data, streampos *line_num_stop){
  _log->writeDebug("function: bool SMIHED::parseDataFile (string file_path, list<Data*> &list_data)");
  const regex reg_comment("^## ([^:]+):?\t?([^\n\\r]*).*");
  const regex reg_keyset("^## ([^\n\\r]*).*");
  ...
  string  subjectID = "";
  int   sizeX=1, sizeY=1;
  float offsetX=0., offsetY=0., ratioX=1., ratioY=1.; 
  string imageName = "";
  bool pos_in_pixels = false, size_in_pixels = false, convert_offset = false;
  const unsigned int pos_time  = 0;
  unsigned int pos_lporx    = 0;
  unsigned int pos_lpory    = 0;
  unsigned int pos_lporx_mm = 0;
  unsigned int pos_lpory_mm = 0;
  unsigned int pos_ldiax    = 0;
  unsigned int pos_ldiay    = 0;
  unsigned int pos_ldia     = 0;
  unsigned int pos_ldiax_mm = 0;
  unsigned int pos_ldiay_mm = 0;
  unsigned int pos_ldia_mm  = 0;
  unsigned int pos_rporx    = 0; 
  unsigned int pos_rpory    = 0;
  unsigned int pos_rporx_mm = 0; 
  unsigned int pos_rpory_mm = 0;
  unsigned int pos_rdiax    = 0;
  unsigned int pos_rdiay    = 0;
  unsigned int pos_rdia     = 0;
  unsigned int pos_rdiax_mm = 0;
  unsigned int pos_rdiay_mm = 0;
  unsigned int pos_rdia_mm  = 0;

  //get offset info from Set file
  ifstream setFile;
  string set_file_path = etbutler::extractFileFolderFromPath(file_path) + FILE_SLASH +
    "Set_" + etbutler::extractFileNameFromPath(file_path) + ".set";
  setFile.open (set_file_path, ios::in );
  if (setFile.is_open()) {
    _log->writeDebug("Getting info from Set file:" + set_file_path);
    ...
    while ( getline (setFile,line) ) {
      ...
      if( regex_match(line, what, reg_keyset) ){ 
        //is a commented line (##)
        if(      what.size() > 1 && what[1].str() == "plane size x"   ){ key = "sizeX";     }
        else if( what.size() > 1 && what[1].str() == "plane size y"   ){ key = "sizeY";     }
        else if( what.size() > 1 && what[1].str() == "plane offset x" ){ key = "offsetX";   }
        else if( what.size() > 1 && what[1].str() == "plane offset y" ){ key = "offsetY";   }
        else if( what.size() > 1 && what[1].str() == "image file"     ){ key = "imageFile"; }
      }else if( key != "" ){
        etbutler::trim(line);
        if(       key == "offsetX"   ) { offsetX = stof(line.c_str()); }
        else if(  key == "offsetY"   ) { offsetY = stof(line.c_str()); }
        else if(  key == "imageFile" ) { imageName = line; }
        else if(  key == "sizeX"     ) { sizeX = atoi(line.c_str());
          value = "imagedimension=" + line;
        }else if( key == "sizeY"     ) { sizeY = atoi(line.c_str());
          value += "x" + line;
    ...
    _log->writeWarn("Cannot find Set file:" + set_file_path);
  }
  _log->writeDebug("SizeX=" + etbutler::int2str(sizeX) +  " "
                   "SizeY=" + etbutler::int2str(sizeY) + " "
                   "OffsetX=" + etbutler::float2str(offsetX) + " "
                   "OffsetY=" + etbutler::float2str(offsetY));
  ...
  if (dataFile.is_open()) {
        ...
        if( line_separate.size() > 1 ){
          if( line_separate[0] == "Time" ){
            ...
            for( unsigned int i = 1; i < line_separate.size(); i++){
              ...
              if( line_separate[i] == "L POR X [mm]"){
                pos_lporx_mm = i;
              }
              else if( line_separate[i] == "L POR Y [mm]"){
                pos_lpory_mm = i;
              }
              ...
              else if( line_separate[i] == "L Dia [px]"){
                pos_ldia = i;
                _pupils = true;
              }
              else if( line_separate[i] == "L Dia X [mm]"){
                pos_ldiax_mm = i;
                _pupils = true;
              }
              else if( line_separate[i] == "L Dia Y [mm]"){
                pos_ldiay_mm = i;
                _pupils = true;
              }
              else if( line_separate[i] == "L Dia [mm]"){
                pos_ldia_mm = i;
                _pupils = true;
              }
              ...
              else if( line_separate[i] == "R POR X [mm]"){
                pos_rporx_mm = i;
              }
              else if( line_separate[i] == "R POR Y [mm]"){
                pos_rpory_mm = i;
              }
              ...
              else if( line_separate[i] == "R Dia [px]"){
                pos_rdia = i;
                _pupils = true;
              }
              else if( line_separate[i] == "R Dia X [mm]"){
                pos_rdiax_mm = i;
                _pupils = true;
              }
              else if( line_separate[i] == "R Dia Y [mm]"){
                pos_rdiay_mm = i;
                _pupils = true;
              }
              else if( line_separate[i] == "R Dia [mm]"){
                pos_rdia_mm = i;
                _pupils = true;
              }
            }
    ...
    //position and size are in pixels or milimiters (if milimiters need to convert)
    if( pos_lporx || pos_rporx ){ 
      pos_in_pixels = true;
      _log->writeDebug("Position in Pixels");
    }else{
      _log->writeDebug("Position in Milimiters");
      if( sizeX == 1 || sizeY == 1 ){
        _log->writeWarn("Cannot translate from milimiters to pixels");
        dataFile.close();
        return false;
      }
    }
    if( pos_ldiax || pos_rdiax || pos_rdia || pos_ldia ){
      size_in_pixels = true;
      _log->writeDebug("Size in Pixels");
    }else{
      _log->writeDebug("Size in Milimiters");
      if( sizeX == 1 || sizeY == 1 ){
        _log->writeWarn("Cannot translate from milimiters to pixels");
        dataFile.close();
        return false;
    ...
    //check if we have to convert the offset to pixels
    if( _use_offset && (offsetX != 0 || offsetY != 0) ){
        convert_offset = true;
        _log->writeDebug("Offset in Milimiters");
        if( sizeX == 1 || sizeY == 1 ){
        _log->writeWarn("Cannot translate from milimiters to pixels");
        _log->writeWarn("Impossible to use offset");
        dataFile.close();
        return false;
      }
    }
    ...
    while ( getline (dataFile,line) ){
      vector<string> line_separate;
      etbutler::split(line, '\t', line_separate);
      if (line_separate.size() < 3){ continue; }
      if( line_separate[1] == "MSG" ){
        ...
        int last = line_separate.size()-1;
        string lineToReg = line_separate[last];
        // cout << endl<< imageName << endl;
        if( imageName != "" ){ lineToReg = "# Message: " + imageName; }
        if( regex_match(lineToReg, what_msg, reg_message) ){
          if( data != NULL ){
          ...
          string image_path = _bilder_path + FILE_SLASH + what_msg[1].str() + "." + what_msg[2].str();
          //need to calculate offset in pixels => need image size in px and mm
          if( !pos_in_pixels || !size_in_pixels || convert_offset ){
            int sizePX, sizePY;
    if( etbutler::getImageSize( image_path , sizePX, sizePY ) && sizePX > 0 && sizePY > 0 ){
              ratioX = ((float)sizePX)/sizeX; ratioY = ((float)sizePY)/sizeY;
              offsetX *= ratioX;
              offsetY *= ratioY;
              ...
              _log->writeWarn("Impossible to use ratio/offset");
              delete(data);
              dataFile.close();
              return false;
            }
          ...
          _log->writeDebug("OffsetPX=" + etbutler::float2str(offsetX) + " OffsetPY=" + etbutler::float2str(offsetY));
        }
        ...
        if( pos_in_pixels ){ //distances are in pixels for POR
          if( offsetX != 0 ){
            if( pos_lporx>0 ) { d_lx = etbutler::float2str(stof(line_separate[pos_lporx].c_str()) + offsetX); }
            if( pos_rporx>0 ) { d_rx = etbutler::float2str(stof(line_separate[pos_rporx].c_str()) + offsetX); }
          }else{
            if( pos_lporx>0 ) { d_lx = line_separate[pos_lporx]; }
            if( pos_rporx>0 ) { d_rx = line_separate[pos_rporx]; }
          }
          if( offsetY != 0 ){
            if( pos_lpory>0 ) { d_ly = etbutler::float2str(stof(line_separate[pos_lpory].c_str()) + offsetY); }
            if( pos_rpory>0 ) { d_ry = etbutler::float2str(stof(line_separate[pos_rpory].c_str()) + offsetY); }
          }else{
            if( pos_lpory>0 ) { d_ly = line_separate[pos_lpory]; }
            if( pos_rpory>0 ) { d_ry = line_separate[pos_rpory]; }
          }
        }else{ //distances are in mm => convert to pixel
          // don't care checking  offset!=0, we have to convert to float anyway
          if( pos_lporx_mm>0 ) { d_lx = etbutler::float2str(stof(line_separate[pos_lporx_mm].c_str())*ratioX + offsetX); }
          if( pos_rporx_mm>0 ) { d_rx = etbutler::float2str(stof(line_separate[pos_rporx_mm].c_str())*ratioX + offsetX); }
          if( pos_lpory_mm>0 ) { d_ly = etbutler::float2str(stof(line_separate[pos_lpory_mm].c_str())*ratioY + offsetY); }
          if( pos_rpory_mm>0 ) { d_ry = etbutler::float2str(stof(line_separate[pos_rpory_mm].c_str())*ratioY + offsetY); }
        }

        //second check the sizes
        if( size_in_pixels ){ //distances are in pixels for DIAMETER
          if( pos_ldia>0 ){
            d_ldiax = line_separate[pos_ldia];
            d_ldiay = line_separate[pos_ldia];
          }
          if( pos_rdia>0 ){
            d_rdiax = line_separate[pos_rdia];
            d_rdiay = line_separate[pos_rdia];
          }
        ...
        }else{ //distances are in mm => convert to pixel => float conversion
          if( pos_ldia_mm>0 ){
            d_ldiax = etbutler::float2str(stof(line_separate[pos_ldia_mm].c_str())*ratioX);
            d_ldiay = etbutler::float2str(stof(line_separate[pos_ldia_mm].c_str())*ratioY);
          }
          if( pos_rdia_mm>0 ){
            d_rdiax = etbutler::float2str(stof(line_separate[pos_rdia_mm].c_str())*ratioX);
            d_rdiay = etbutler::float2str(stof(line_separate[pos_rdia_mm].c_str())*ratioY);
          }
          if( pos_ldiax_mm>0 )  { d_ldiax = etbutler::float2str(stof(line_separate[pos_ldiax_mm].c_str())*ratioX); }
          if( pos_rdiax_mm>0 )  { d_rdiax = etbutler::float2str(stof(line_separate[pos_rdiax_mm].c_str())*ratioX); }
          if( pos_ldiay_mm>0 )  { d_ldiay = etbutler::float2str(stof(line_separate[pos_ldiay_mm].c_str())*ratioY); }
          if( pos_rdiay_mm>0 )  { d_rdiay = etbutler::float2str(stof(line_separate[pos_rdiay_mm].c_str())*ratioY); }
        }
        ...
        // cout << " d_lx    "  << d_lx    << endl;
        // cout << " d_ly    "  << d_ly    << endl;
        // cout << " d_ldiax "  << d_ldiax << endl;
        // cout << " d_ldiay "  << d_ldiay << endl;
        // cout << " d_rx    "  << d_rx    << endl;
        // cout << " d_ry    "  << d_ry    << endl;
        // cout << " d_rdiax "  << d_rdiax << endl;
        // cout << " d_rdiay "  << d_rdiay << endl;
...
bool SMIHED::calculateQuality (list<Data*> &list_data, list<string> &list_quality){
  _log->writeDebug("function: bool SMIHED::calculateQuality (list<Data*> &list_data, list<string> &list_quality)");
    ...
    for(subitr = sublist_data->begin(), subprev1 = sublist_data->end(), subprev2 = sublist_data->end();
      ...
      if( !leftvalid ){
        ...
        if( _pupils && subprev1 != sublist_data->end() && subprev2 != sublist_data->end()){
          //check if pupil height is decreasing (or 0)
...
bool SMIHED::writeQualityHeader (string output_path, string file_prefix){
  _log->writeDebug("SMIHED::writeQualityHeader (string output_path, string file_prefix)");
...
bool SMIHED::writeQualityOutput (list<string> &list_quality){
  _log->writeDebug("bool SMIHED::writeQualityOutput (list<string> &list_quality)");
  ofstream qualityFile;
